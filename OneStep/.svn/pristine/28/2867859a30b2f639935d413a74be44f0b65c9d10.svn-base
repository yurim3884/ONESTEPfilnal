...
//                     str += "<span class='chat-count'>99</span>";
	...
	// 불러온 리스트를 화면에 뿌려줌
	$("#chatList > div").on("click", "#chatDetail", function(event) {	// "chatList" ID를 가진 div 요소 내의 "chatDetail" ID를 가진 요소에 대한 클릭 이벤트 리스너
...
// 		chatroomId = $(this).data("code");
		otherId = event.target.parentNode.parentNode.getAttribute( 'data-id' );
		const obj = {
 			'chatroomId' : otherId,
			'memId' : memId
		...
		$.ajax({
		    ...
		    success: function(res) {
		        ...
		        let str = "";
		        let cnt = 0;
		        $("#chatName").html(otherId+"님과의 대화");

		        if (res.length > 0) {	// 서버에서 반환된 데이터의 길이가 0보다 크면 (데이터가 존재하면)
		            $.each(res, function(index, item) {
		                const chatMemId = '${item.chatMemId}';
		                console.log('${item.chatMemId}')
