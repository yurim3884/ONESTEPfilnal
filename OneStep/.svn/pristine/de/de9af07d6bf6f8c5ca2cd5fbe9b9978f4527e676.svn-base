									...
									<tbody id="tbody">
										...
										<c:forEach items="${annoList}" var="annoList">
											...
											<tr>
												...
												<td style="width: 300px;"><fmt:formatDate pattern="yyyy-MM-dd " value="${annoList.annoStartDate}" /></td>
	...
	$(function(){
		...
		tbody.on("click",function(event){
			...
			if(event.target.id=='modalBtn'){
				...
				var annoSalary = event.target.parentNode.parentNode.childNodes[7].innerText;
				var obj = {annoSalary: annoSalary};
